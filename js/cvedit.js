...
cvmutual.main = {
	...
	event_:function(){// 事件
        ...
        document.addEventListener("paste", function(e) {
            ...
            if($(":focus").is("div[contenteditable=true]")) {
                ...
                if($(":focus").hasClass("module_item_title") || $(":focus").parents(".dd-title").length > 0){
                    var _length = $(":focus").text().length;
                    if(_length > 30){
                        $(":focus").text($(":focus").text().substring(0,29));
                        layer.msg("你输入的字数不能超过30");
                    }
                }
                e.preventDefault();
            }
        ...
        $(document).on("click", ".case_bar", function(){
        	var _href="/cvresume/cases/?itemid="+cvresume.info.itemid;
        	if(cvresume.info.resumeid>0){
        		_href=_href+"&resumeId="+cvresume.info.resumeid;
        	}
		...
		$(document).ready(function(){ 
			...
			$(".wbdCv-baseStyle").each(function(){
                ...
                if($(this).attr("data_font_name") ==""){
                    $("#myselect").val("微软雅黑");
                    $("#select-font").text("微软雅黑");
                }else{
                    var $font_name = $(" #setFontfamily li[data-fontname = "+_data_font_name+"]").text();
                    $("#myselect").val($font_name);
                    $("#select-font").text($font_name);
                }
                if($(this).attr("data_font_size") ==""){
                    $("#fonts_amount").val("14");
                }else{
                    $("#fonts_amount").val(_data_font_size);
                    var left = (_data_font_size-12)*(1/6)*100 + "%";
                    $("#fonts_slider .ui-slider-handle").css("left",left)
                }
	    ...
	    $( "#fonts_slider" ).slider({
	      ...
	      slide: function( event, ui ) {
	        ...
	        cvmutual.main.set_fgsz_fontsize(ui.value);
	      },
	      change:function(event,ui){
	      	cvresume.main.delay_resume_save();
	      	common.main._500dtongji("PC-在线制作-风格设置功能（"+cvmutual.info.resume_type+"编辑）-字体大小调整-字体大小调整-调整到某大小");
	      }
	    });	
		// 滑块设置段距高
	    $( "#lineh_slider" ).slider({
	...
	resume_select_template:function(){
		 ...
		 var tagId;
		$(document).on('click',"#change_temlate_btn:not(.wbd-vip-lock)",function(){
			// 1判断是否存在模板节点
			if($("#data_template_list li").length<=0){
				// 无节点，发送ajax请求渲染回ul
				reload_template_list();
			}
			$("#changeResumeModal").modal("show");
			cvmutual.main.close_tips_event();
		})	
		// 根据标签选择模板
		$('.change_modal_filter span').click(function(){
	    	$(this).addClass("current").siblings().removeClass("current");
	    	select_template_page=1;
	    	tagId=$(this).attr("data_id");
	    	reload_template_list();
	    })
		// 搜索
		...
		function reload_template_list(){
			...
			if(select_template_page==1){
				$("#data_template_list").load("/cvresume/select_template/",{"type":current_resume_type,"tagId":tagId,"resumeBankType":current_resume_bank_type,"pageNum":select_template_page,"sort":data_sort,"keyword":keyword},function(){
					...
					cvmutual.main.set_changeModal_mouse($(".zx-mblist-box .list-con"));
				});
			}else{
				 $.get("/cvresume/select_template/",{"type":current_resume_type,"tagId":tagId,"resumeBankType":current_resume_bank_type,"pageNumber":select_template_page,"sort":data_sort,"keyword":keyword},function(result){
		...
		$(document).on("click","#changeResumeModal .resume_change_template",function(){
	...
	resume_modal:function(){//modal模态框调用
		...
		function modail_listener(e){
		    var $this = $(e.target);
		    if($this.parents("#setStyleModal").length<=0 && $this.parents("#resume_modal_manager").length<=0 && $this.parents("#cvListModal").length<=0 && $this.parents("#historyModal").length<=0){
                $(".litemodal").css('left','-240px');
                $(document).off("click",modail_listener);
            }
            e.stopPropagation();
        }
		// 调用操作历史弹框
		$(".czls a").click(function(e){
            ...
            $(document).on("click",modail_listener);
            var data_value = $(this).attr("data-value");
            common.main._500dtongji("PC-在线制作-"+data_value+"-左侧设置-左侧设置-操作历史");
            e.stopPropagation();
		});		
		// 调用风格设置弹框
	...
	resume_delete:function(){// 删除模块
		function resumeDelete(){
			...
			}else if(key == "resume_skill"){//技能特长删除
				var data_id=$(".baseItem a.delete.selected").closest(".moduleItemList").attr("data-id");
		...
		function resumeHidden(){
			// baseItem模块隐藏
			 ...
			 if($this.hasClass("customItem")){
			 	$this.remove();
			 	$forid.find("li.custom-li").remove();
			 	 $("#delete-modal").modal("hide");
			 }else{
				 $this.addClass("hidden");
				 $("#delete-modal").modal("hide");			 	
	...
	set_changeModal_mouse:function(obj){// 更换模板鼠标移动效果
		obj.each(function(){
	    ...
	    function direct(e,o){
		 ...
		 var angle=(Math.round((Math.atan2(y,x)*(180/Math.PI)+180)/90)+3)%4 // atan2返回的是弧度
	...
	set_infoItem:function(){
        ...
        $("#baseMsg-modal .modal-footer .submit").click(function(){
			...
			if(missAge || missDefind || missSelf || NaN || check_input){
                ...
                if($("[data-panel = 'wx']").val() == ""){
                   $(".social-wx a").attr("data-title","微信");
                   $(".social-wx").addClass("hidden");
                }else{
                   $(".social-wx a").attr("data-title","微信：" + $("[data-panel = 'wx']").val());
                   $(".social-wx").removeClass("hidden");
                }       // wx
                if($("[data-panel = 'qq']").val() == ""){
                   $(".social-qq a").attr("data-title","QQ");
                   $(".social-qq").addClass("hidden");
                }else{
                   $(".social-qq a").attr("data-title","QQ：" + $("[data-panel = 'qq']").val());
                   $(".social-qq").removeClass("hidden");
                }       // qq
                if($("[data-panel = 'dd']").val() == ""){
                   $(".social-dd a").attr("data-title","钉钉号");
                   $(".social-dd").addClass("hidden");
                }else{
                   $(".social-dd a").attr("data-title","钉钉号：" + $("[data-panel = 'dd']").val());
                   $(".social-dd").removeClass("hidden");
                }       // dd
                if($("[data-panel = 'zh']").val() == ""){
                   $(".social-zh a").attr("href","#");
                   $(".social-zh").addClass("hidden");
                }else{
                   var zh = $("[data-panel = 'zh']").val();
                   if(zh.indexOf("http://") < 0 && zh.indexOf("https://") < 0 ){
                       $(".social-zh a").attr("href","http://" + zh);
                       $("[data-panel = 'zh']").val("http://" + zh);
                       $(".social-zh").removeClass("hidden");
                   }else{
                       $(".social-zh a").attr("href",zh);
                       $(".social-zh").removeClass("hidden");
                   }
                }       // zh
                if($("[data-panel = 'wb']").val() == ""){
                   $(".social-wb a").attr("href","#");
                   $(".social-wb").addClass("hidden");
                }else{
                   var wb = $("[data-panel = 'wb']").val();
                   if(wb.indexOf("http://") < 0 && wb.indexOf("https://") < 0){
                       $(".social-wb a").attr("href","http://" + wb);
                       $("[data-panel = 'wb']").val("http://" + wb);
                       $(".social-wb").removeClass("hidden");
                   }else{
                       $(".social-wb a").attr("href",wb);
                       $(".social-wb").removeClass("hidden");
                   }
                }       // wb
                ...
                if($("[data-panel='dd']").val() == "" && $("[data-panel='wx']").val() == "" && $("[data-panel='qq']").val() == "" && $("[data-panel= 'zh']").val() == "" && $("[data-panel= 'wb']").val() == ""){
                $(".socialItem").addClass("hidden");
                $(".socialItem").removeClass("current");
				$(".curr_bg").css('display','none');
				$(".moduleItem").removeClass("current");
                }else{
		...
		$("#add_social").on('click',function(){
			if($(".chose_social_modal").css("display") == "block"){
                $(".chose_social_modal").fadeOut();
            }else{
                $(".social_bar li").each(function(){
                    var type = $(this).find("input").attr("data-panel");
                    if($(this).hasClass("hidden")){
                        $(".chose_social li[data-social="+type+"]").removeClass("checked")
                    }else{
                        $(".chose_social li[data-social="+type+"]").addClass("checked")
                    }
                });
                $(".chose_social_modal").fadeIn();
			}
		}); //		添加社交账号按钮点击事件（同步已选择显示的社交项）
		$(".chose_social_content .social_list").on('click',function(){
			$(this).toggleClass("checked");
		}); //	选择面板上的社交项点击事件
		...
		$(".social_list .social_list_footer").on('click',function(){
			var $this = $(this).parents(".social_list ");
			$this.addClass("hidden");
			$this.find("input").val("");
		...
		$("#addSelf").on('click',function(){
            var key = cvmutual.main.makeId();
			var $self = $('<li class="self_list" data-panel="homePage"></li>').attr("data-value",key).html('<div class="self_list_header"></div>' +
				'<div class="self_list_body"><input type="text" name="homeDesc" placeholder="添加主页描述（10字以内）" maxlength="10" /><input type="text" name="homeUrl" placeholder="输入个人主页" /></div>' +
				'<div class="self_list_footer"></div>');
			$self.appendTo($(".self_bar"))
		}); //	添加个人主页按钮点击事件
		$(document).on('click','.self_list .self_list_footer',function(){
			$(this).parent(".self_list").remove()
		}) //	个人主页删除按钮
	},
	set_inteItem:function(){
		...
		$("#jobIntension-modal .modal-footer .submit").click(function(){
            ...
            if(missPrice || wormInput || $NaN){
                ...
                var inteJob, inteType, inteCity, intePrice, inteTime;
                inteJob = $("[data-panel = 'inteJob']").val() == "" ? "" : $("[data-panel = 'inteJob']").val();
                inteType = $("[data-panel = 'inteType']").prev("span").text() == "选择职业类型" ? "" : $("[data-panel = inteType]").prev("span").text();
                inteCity = $("[data-panel = 'chosecity']").prev().prev("span").text() == "选择意向城市" ? "" : $("[data-panel = 'chosecity']").prevAll("span").text();
                inteTime = $("[data-panel = 'inteTime']").prev("span").text() == "选择入职时间" ? "" : $("[data-panel = 'inteTime']").prev("span").text();
                if(cvresume.info.language=="en"){		
                ...
                //判断意向职位val()是否空
                $(".inte-job span").text(inteJob);
                //判断意向城市val()是否空
                $(".inte-city span").text(inteCity).attr("data-value",$("[data-panel = 'chosecity']").val());                              
                //判断工作类型是否null
                if( $("[data-panel='inteType']").attr("value") == ""){
                   $(".inte-type span").text("").attr("data-value",$("[data-panel = 'inteType']").val());
                }else{
                    $(".inte-type span").text(inteType).attr("data-value",$("[data-panel = 'inteType']").val());
                }
                //判断最快到岗时间是否null
                if( $("[data-panel='inteTime']").attr("value") == ""){
                   $(".inte-time span").text("").attr("data-value",$("[data-panel = 'inteTime']").val())
                }else{
                    $(".inte-time span").text(inteTime).attr("data-value",$("[data-panel = 'inteTime']").val())
                }         
    ...
    set_skillItem:function(){
        ...
        $(document).on("click",".skillItem .edit,.skillItem .baseItem-null,.skillItem .skill-title",function(){
            ...
            if($("#skillistBody a").length==0){
            	$.get("/cvresume/skill/",function(result){
				if(result!=null&&result!=""){
	...
	set_hobbyItem:function(){
        ...
        $("#hobbys-modal .skillInput").on('keypress',function(event){
	        ...
	        $(document).on("click",".hobbyItem .edit,.hobbyItem .baseItem-null,.hobbyItem .hobby-title",function(){
	            ...
	            if($("#hobby a").length==0){
	            	$.get("/cvresume/hobby/",function(result){
	            		if(result!=null&&result!=""){
	            			$("#hobby").append(result);
	            			if($("#hobby a").length!=0 ){
    ...
    get_infoToModal:function(){
        ...
        if($(".social-wx a").attr("data-title") == "微信"){
            $("[data-panel=wx]").val("");
            $("[data-panel=wx]").parents(".social_list ").addClass('hidden')
        }else{
            var wx = $(".social-wx a").attr("data-title"), wxIndex = wx.indexOf("：")+1;
            $("[data-panel=wx]").val(wx.substring(wxIndex));
            $("[data-panel=wx]").parents(".social_list ").removeClass('hidden')
        }       // wx
        if($(".social-qq a").attr("data-title") == "QQ"){
            $("[data-panel=qq]").val("");
            $("[data-panel=qq]").parents(".social_list ").addClass('hidden')
        }else{
            var qq = $(".social-qq a").attr("data-title"),qqIndex = qq.indexOf("：")+1;
            $("[data-panel=qq]").val(qq.substring(qqIndex));
            $("[data-panel=qq]").parents(".social_list ").removeClass('hidden')
        }       // qq
        if($(".social-zh a").attr("href") == "#"){
            $("[data-panel= zh]").val("");
            $("[data-panel=zh]").parents(".social_list ").addClass('hidden')
        }else{
            var zh = $(".social-zh a").attr("href");
            $("[data-panel= zh]").val(zh);
            $("[data-panel=zh]").parents(".social_list ").removeClass('hidden');
        }       // zh
        if($(".social-wb a").attr("href") == "#"){
            $("[data-panel= wb]").val("");
            $("[data-panel=wb]").parents(".social_list ").addClass('hidden');
        }else{
            var wb = $(".social-wb a").attr("href");
            $("[data-panel= wb]").val(wb);
            $("[data-panel=wb]").parents(".social_list ").removeClass('hidden');
        }       // wb
        if($(".social-dd a").attr("data-title") == "钉钉号"){
            $("[data-panel=dd]").val("");
            $("[data-panel=dd]").parents(".social_list ").addClass('hidden')
        }else{
            var dd = $(".social-dd a").attr("data-title"), ddIndex = dd.indexOf("：")+1;
            $("[data-panel=dd]").val(dd.substring(ddIndex));
            $("[data-panel=dd]").parents(".social_list ").removeClass('hidden')
        }       // dd
	},
	...
	caclulate_resume_scale:function(resume){// 计算简历制作进度
		...
		try{
			// 基本信息（20）
			// 默认信息
			if(common.main.isEffect(resume.resume_base_info.name,org_resume.resume_base_info.name)){//姓名（3）
				grade+=3;
			}
			if(common.main.isEffect(resume.resume_base_info.birthYear,org_resume.resume_base_info.birthYear)){//生日（3）
				grade+=2;
			}
			if(common.main.isEffect(resume.resume_base_info.cityName,org_resume.resume_base_info.cityName)){//所在城市（2）
				grade+=2;
			}
			if(common.main.isEffect(resume.resume_base_info.jobYear,org_resume.resume_base_info.jobYear)){//工作年限（2）
				grade+=3;
			}
			if(common.main.isEffect(resume.resume_base_info.mobile,org_resume.resume_base_info.mobile)){//电话号码（3）
				grade+=2;
			}
			if(common.main.isEffect(resume.resume_base_info.email,org_resume.resume_base_info.email)){//联系邮箱（1）
				grade+=2;
			}
			if(common.main.isEffect(resume.resume_base_info.minSummary,org_resume.resume_base_info.minSummary)){//一句话介绍自己（1）
				grade+=1;
			}
			if(grade != 0){// 当默认信息有填写时，补充信息分数才生效
				// 补充信息
				if(common.main.isEffect(resume.resume_base_info.sex,org_resume.resume_base_info.sex)){//性别（1）
					grade+=1;
				}
				if(common.main.isEffect(resume.resume_base_info.education,org_resume.resume_base_info.education)){//学历（1）
					grade+=1;
				}
				if(common.main.isEffect(resume.resume_base_info.nation,org_resume.resume_base_info.nation)){//民族（1）
					grade+=1;
				}
				if(common.main.isEffect(resume.resume_base_info.marriageStatus,org_resume.resume_base_info.marriageStatus)){//婚姻（1）
					grade+=1;
				}
				if(common.main.isEffect(resume.resume_base_info.politicalStatus,org_resume.resume_base_info.politicalStatus)){//政治面貌（1）
					grade+=1;
				}
				if(common.main.isEffect(resume.resume_base_info.weight,org_resume.resume_base_info.weight) || common.main.isEffect(resume.resume_base_info.height,org_resume.resume_base_info.height)){// 身高体重1%
					grade+=1;
				}
				if(resume.resume_base_info.customWebsite.length > 0){//个人主页（1）
					grade+=1;
				}
				if(!cvresume.main.is_empty(resume.resume_base_info.weixin) || !cvresume.main.is_empty(resume.resume_base_info.qq)
				|| !cvresume.main.is_empty(resume.resume_base_info.zhihu) || !cvresume.main.is_empty(resume.resume_base_info.weibo)
				|| !cvresume.main.is_empty(resume.resume_base_info.dingding)){//社交账号（1）
					grade+=1;
				}
			}
			// 头像（5）
			if((resume.resume_head.indexOf("1.jpg") == -1 && resume.modul_show.headShow == true)&&common.main.isEffect(resume.resume_head,org_resume.resume_head)){
				grade+=5;
			}
			// 求职信息（15）
			if(common.main.isEffect(resume.resume_job_preference.jobFunction,org_resume.resume_base_info.jobFunction)){
				grade+=3;
			}
			if(common.main.isEffect(resume.resume_job_preference.jobType,org_resume.resume_base_info.jobType)){
				grade+=3;
			}
			if(common.main.isEffect(resume.resume_job_preference.jobMinSalary,org_resume.resume_base_info.jobMinSalary)){
				grade+=3;
			}
			if(common.main.isEffect(resume.resume_job_preference.jobCityName,org_resume.resume_base_info.jobCityName)){
				grade+=3;
			}
			if(common.main.isEffect(resume.resume_job_preference.jobTime,org_resume.resume_base_info.jobTime)){
				grade+=3;
			}
			// 教育背景或工作经验或实习经验
			if(resume.modul_show.resume_edu.isShow == true&&common.main.isEffect(resume.resume_edu,org_resume.resume_edu)){
				grade+=10;
			}
			if(resume.modul_show.resume_work.isShow == true&&common.main.isEffect(resume.resume_work,org_resume.resume_work)){
				grade+=15;
			}
			if(resume.modul_show.resume_internship.isShow == true&&common.main.isEffect(resume.resume_internship,org_resume.resume_internship)){
				grade+=15;
			}
			// 自愿者经验（2）
			if(resume.modul_show.resume_volunteer.isShow == true&&common.main.isEffect(resume.resume_volunteer,org_resume.resume_volunteer)){
				grade+=8;
			}
			// 项目经验（2）
			if(resume.modul_show.resume_project.isShow == true&&common.main.isEffect(resume.resume_project,org_resume.resume_project)){
				grade+=10;
			}
			// 自我评价（4）
			if(common.main.isEffect(resume.resume_summary,org_resume.resume_summary) && resume.modul_show.resume_summary.isShow == true){
				grade+=15;
			}
			// 奖项荣誉（4）
			if(common.main.isEffect(resume.resume_honor,org_resume.resume_honor) && resume.modul_show.resume_honor.isShow == true){
				grade+=10;
			}
			// 兴趣（2）
			if(resume.modul_show.resume_hobby.isShow == true&&common.main.isEffect(resume.resume_hobby,org_resume.resume_hobby)){
				grade+=8;
			}
			// 特长（2）
			if(resume.modul_show.resume_skill.isShow == true&&common.main.isEffect(resume.resume_skill,org_resume.resume_skill)){
				grade+=8;
			}
			// 作品展示（4）
			if((common.main.isEffect(resume.resume_portfolio.img,org_resume.resume_portfolio.img)||common.main.isEffect(resume.resume_portfolio.link,org_resume.resume_portfolio.link)) && resume.modul_show.resume_portfolio.isShow == true){
				grade+=8;
			}
			// 联系我（2）
			if(common.main.isEffect(resume.modul_show.contactShow,org_resume.modul_show.contactShow)){
			}
			var _falseGrade;
			if(grade <= 80){
				_falseGrade = grade;
			}else{//80-150
				_falseGrade = 80 + Math.ceil((grade - 80) * 2 / 7);
			}
    ...
    tips_event:function(){//小贴士       
        ...
        $(".tips_content_text").mouseenter(function(){
        	 $("body").addClass("modal-open");
        });      
        $(".tips_content_text").mouseleave(function(){
        	 $("body").removeClass("modal-open");
        });        
        //点击小贴士关闭按钮事件
