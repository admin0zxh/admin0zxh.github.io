	    ...
	    event_: function () {//全局事件绑定
	    	...
	    	if(getCookie("memberEmail")||getCookie("memberName")){
	    		$.get(wbdcnf.base+"/common/get_message_notification_count/",function(data){
	    			...
	    			//过年消息通知处理
	    			try{
	    				if(window.localStorage && common.main.is_empty(localStorage.getItem("year_message_notify"))){
	    					$message_notification.show();
	    					if(!common.main.is_empty($("#year_message_notify"))){
	    						$("#year_message_notify").addClass("red");
	    					}
	    				}
	    			}catch(e){
	    				console.log("过年消息通知处理异常-----"+e);
	    			}
	    		});
	    	}else{
	    		$("#user_center i").hide();
...
//            common.main.text_maxlength($("#search_btn"),200); 
		...
		set_message_notification_count: function(type,id){
			$.get("/member/message_notification/count/",{"type":type},function(data){
				try{
					if(window.localStorage && common.main.is_empty(localStorage.getItem("year_message_notify"))&&id=="sysTips"){
						data=data+1;
    				}
				}catch(e){
					console.log("处理年假消息通知失败："+e);
				}
				if(data>0){
					$("#"+id).text(data);
